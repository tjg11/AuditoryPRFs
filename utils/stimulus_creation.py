...
# Description: Designed to take in a matlab stimulus for auditory trials and
# convert them to either image or binary files for use in following scripts.
# Stimulus space for this experiment in audio (left -> right) represented in a
# numerical array starting at -30 and ending at 30 with 9 steps between the
# two. Stimuli can either travel from left to right or right to left, and fall
# into 3 categories: sequential, onset/offset, and random.
...
def stimulus_creation(in_path, save_type):
    """Creates and saves out stimuli in binary or img format.
    Path to data to be transformed and saved
    entered in in_path. Type of file saved out defined by
    save_type (1=binary, 2=binary[with actual dists], 3=image)"""
    ...
    print(f"""
          Conditions for random mat file (category, direction): {cond_as_numpy}
          """)
    ...
    for mat_idx in range(len(all_matfiles)):
        ...
        subject_name = re.sub(r"^(sub-[\w\d]+)_.+", "\\1", bname)
    ...
    if save_type == 1:
        ...
        for mat_idx in range(len(all_matfiles)):
            ...
            stim_record = np.zeros((int(n_bins), S_LEN))
                    ...
                    if sample_x < 9:
                        ...
                        if n_burst <= 8:
                            ...
                            if sample_x < 9 and times_rep != max_reps:
                                ...
                                n_burst = 1
                                target = cond_as_numpy[sample_y, 0]
                                target_idx = int(target - 1)
                                dist = DIST[target_idx]
                                if FLIP == -1:
                                    target_idx = (abs(target_idx - S_LEN)) - 1
            ...
            bname = op.basename(fname)
            subject_name = re.sub(r"^(sub-[\w\d]+)_.+", "\\1", bname)
            run_index = re.sub(r".+_run-(\d+)_.+", "\\1", bname)
            img_name = 'binary_' + subject_name + '_0' + run_index + ".pickle"
        ...
        # Get the pickle file for each mat file in matfiles (dist)
        S_LEN = 9
        TRIAL_LEN = 26 * 16000  # ms
        RESOLUTION = 10  # ms
        DIR_LOC = 2  # index in conditions
        ...
        for mat_idx in range(len(all_matfiles)):
                ...
                if times_rep == max_reps:
                    ...
                    if sample_x < 9:
                        ...
                        if n_burst <= 8:
                            ...
                            stim_record[sr_row, target_idx] = dist * FLIP
                            n_burst += 1
                        else:
                            sample_x += 1
                            n_burst = 0
            ...
            subject_name = re.sub(r"^(sub-[\w\d]+)_.+", "\\1", bname)
            run_index = re.sub(r".+_run-(\d+)_.+", "\\1", bname)
            img_name = subject_name + '_' + '0' + run_index + ".pickle"
            img_name = op.join(subject_name, img_name)
            with open(img_name, "wb") as f:
                pickle.dump(stim_record, f)
        toc = time.perf_counter()
        ...
        for mat_idx in range(len(all_matfiles)):
            ...
            subject_name = re.sub(r"^(sub-[\w\d]+)_.+", "\\1", bname)
            run_index = re.sub(r".+_run-(\d+)_.+", "\\1", bname)
            img_name = subject_name + '_0' + run_index + ".png"
            img_name = op.join(subject_name, img_name)
