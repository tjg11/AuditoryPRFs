...
if sanity_check:
    print("Total length of run should be 416 seconds.")
    print("Total rows per secound should be 100.")
    print("Shape should be (41600, 9).")
...
# Error function: get mode, prediction, and calculate error between prediction
# and real data
...
if sanity_check:
    print("Total length of run should be 430 seconds.")
    print("Total rows per second should be 100.")
    print("Shape should be (43000, 9).")
...
grid_coords = np.vstack([xv.flatten(), yv.flatten()]).T
...
tic = time.time()
for z_coord in range(z_new_start, z_total):
    done_voxel = 0
    all_results = []
    ...
    for coord in grid_coords:  # for each coordinate in voxel grid
        ...
        x, y = coord
        vox = bold_data[x, y, z_coord, :]
        norm_voxel = (vox - np.amin(vox)) / (np.amax(vox) - np.amin(vox))

        print(f"\n-----Starting for {x, y, z_coord} voxel------\n")

    # Generate multiple gaussians and find best fit - this also needs to be
    # done in loop since voxel will be different
        ...
        for i, params in enumerate(all_params):
            ...
            corr = pearsonr(vox, pred_t).statistic
            # print(corr)
            error_matrix[i] = -corr
        ...
        if print_status:
            print(f"Min error is at {min_x[0]}, {min_y[0]}.")
            print(f"This is a mu of {mus[min_x[0]][min_y[0]]}.")
            print(f"This is a sigma of {sigmas[min_x[0]][min_y[0]]}.")
        ...
        # (the best_mu and best_si values can be found wihtout manually
        # entering them, I just need to write the code for it)
        ...
        grid_results = dict()
        grid_results['voxel'] = [x, y, z_coord]
    ...
    print("--------")
    img_name = op.join(base_path, f"prf_results_{z_coord}.pickle")
    with open(img_name, "wb") as f:
        pickle.dump(all_results, f)
    print(f"FINISHED SLICE #{z_coord}/{z_total - 1}----------")
